 ...
 *   GET  /v1/organs/status?nodeId=gateway  — estado config órganos (sin secretos)
 */
...
import { getOrganStatusReport } from './organ-status.mjs';
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && (req.url === '/health' || req.url === '/v1/health')) {
    json(res, 200, {
      ok: true,
      service: 'agenft-lab-bridge',
      inbox: INBOX,
      version: 2,
      features: ['organs/status', 'wiring', 'inbox'],
    });
    return;
  }
  ...
  if (req.method === 'GET' && req.url?.startsWith('/v1/organs/status')) {
    if (!checkToken(req, res)) return;
    try {
      const url = new URL(req.url, `http://${HOST}`);
      const ctx = resolveAgentEnv();
      const nodeId = url.searchParams.get('nodeId');
      const report = await getOrganStatusReport(ctx, { nodeId: nodeId || null });
      json(res, 200, report);
    } catch (e) {
      json(res, 500, { ok: false, error: e.message ?? String(e) });
    }
