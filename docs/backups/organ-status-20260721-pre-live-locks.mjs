...
function gatewayTelegramStatus(wiring) {
  ...
  const checks = [
    ...
    check('token', !wired || telegramTokenPresent(), 'Token bot Telegram', wired ? undefined : 'Opcional si no cableado'),
    check(
      'adapter',
      wired,
      'Adaptador runtime',
...
async function chatWebStatus(wiring) {
  ...
  const checks = [
    check('wire', wired, 'Cableado al Motor'),
    check('service', !wired || health.ok, 'Servicio chat-api', health.detail ?? `:${CHAT_API_PORT}`),
  ];
  const steps = [
