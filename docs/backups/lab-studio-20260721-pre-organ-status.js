 ...
 */
import { assetUrl, fetchJson, tokenFromPath, applyAvatar } from './shared.js';
import {
...
function renderAlertHtml(alert) {
  return `<div class="lab-alert lab-alert-${alert.level}" role="alert"><strong>${escapeHtml(alert.title)}</strong>${escapeHtml(alert.body)}</div>`;
}
...
function renderFeasibilityAlerts(focusNodeId = null) {
  ...
  const allAlerts = collectFeasibilityAlerts();
  const focusAlerts = focusNodeId ? collectFeasibilityAlerts(focusNodeId) : [];

  if (globalRoot) {
    globalRoot.innerHTML = allAlerts.map(renderAlertHtml).join('');
  }
  if (popoverRoot) {
    popoverRoot.innerHTML = focusAlerts.map(renderAlertHtml).join('');
  }
...
function confirmIfFeasibilityWarnings(actionLabel) {
  const warns = collectFeasibilityAlerts().filter((a) => a.level === 'warn');
...
export async function initLabStudio() {
  ...
  window.addEventListener('storage', (ev) => {
    if (ev.key !== ORGAN_SCALE_KEY) return;
    bindOrganScaleControl(
      document.getElementById('lab-organ-scale'),
      document.getElementById('lab-organ-scale-val'),
      null
...
function renderNodeEditor() {
  ...
  root.innerHTML = `
    <div class="lab-popover-head">
      <h3 class="lab-panel-title">${escapeHtml(node.label)}</h3>
      <button type="button" class="lab-popover-close" id="lab-popover-close" aria-label="Cerrar editor">×</button>
    </div>
  ...
  root.querySelector('#lab-popover-close')?.addEventListener('click', (ev) => {
    ev.stopPropagation();
...
function bindLabPanels() {
  ...
  document.querySelectorAll('[data-close-panel]').forEach((btn) => {
    btn.addEventListener('click', (ev) => {
      ev.stopPropagation();
      const panel = btn.closest('.lab-panel');
...
function renderAssistant() {
  ...
  const warnCount = collectFeasibilityAlerts().filter((a) => a.level === 'warn').length;
  if (warnCount) {
    tips.push(
      '<hr class="lab-hr" />',
...
function renderBridgeHelp(bridgeOnline = false) {
  ...
  root.innerHTML = [
    ...
    '<p><strong>Advertencias</strong> — si algo no es posible hoy (gateway sin bot, órgano sin runtime…), aparece en <span class="status-warn">amarillo</span> bajo el header y en el editor del módulo.</p>',
