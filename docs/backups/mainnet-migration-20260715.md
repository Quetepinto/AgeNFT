...
## Estado actual (lab Sepolia + pagos mainnet)

| Pieza | Hoy | Problema |
|-------|-----|----------|
| Unit-1 #115 | Base Sepolia (VIMS) | No es mainnet |
| TBA `0x2FF43…` | Contrato Sepolia | No existe en mainnet |
| Cerebro tx402.ai | Base mainnet USDC | ✅ funciona |
| Pagador real | EOA lab ~0.045 USDC | ❌ no soberano |
| Memoria | lab-remote + local | ⚠️ no IPFS estable |
| VIMS mint | Solo Sepolia | ❌ no escala |

Reporte prueba TBA: [`lab/tba-x402-pay-report.json`](lab/tba-x402-pay-report.json) — veredicto **PARTIAL**.
...
| A2 | Fork/adapt Agent-NFT **o** deploy desde cero (AGPL review) | `contracts/` |
| A3 | Deploy Base mainnet (o fork test local) | addresses.json |
| A4 | Script `mint-mainnet.mjs` | agentId + TBA + tx hash |
...
| # | Tarea | Entregable |
|---|-------|------------|
| B1 | Manifiesto `unit-mainnet.json` chain 8453 | `docs/manifest/examples/` |
| B2 | `payer-key.mjs` → resolver TBA + session key | pago desde tesoro |
...
| # | Tarea | Notas |
|---|-------|-------|
| C1 | Wallet proyecto: ETH mainnet gas | ~0.002 ETH mint + fund |
| C2 | USDC → TBA (≥5 USDC operativo) | desde EOA proyecto una vez |
| C3 | `tba-x402-pay.mjs` → verde TBA | cerrar item defer checklist |
| C4 | Transfer round-trip mainnet | checklist 8/8 |
| C5 | Telegram bot apunta a Unit-mainnet | nuevo tokenId |

**Swap:** hoy EOA tiene USDC (~0.045) sin ETH mainnet. Opciones:
- Puente Sepolia ETH → mainnet (Across, oficial Base bridge)
- Enviar ETH mainnet directo a wallet proyecto
- **No** hace falta swap USDC si ya hay USDC mainnet
