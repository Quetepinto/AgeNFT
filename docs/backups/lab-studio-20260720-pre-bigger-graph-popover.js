...
function portPoint(node, side) {
  const cx = node.x + NODE_SIZE.w / 2;
  const cy = node.y + NODE_SIZE.h / 2;
  switch (side) {
    case 'top':
      return { x: cx, y: node.y, side };
    case 'bottom':
      return { x: cx, y: node.y + NODE_SIZE.h, side };
    case 'left':
      return { x: node.x, y: cy, side };
    default:
      return { x: node.x + NODE_SIZE.w, y: cy, side: 'right' };
  }
}
...
function nodePortPrefs(node) {
  ...
  const cx = node.x + NODE_SIZE.w / 2;
  const isLeftLimb = g === 'limb' && cx < 440;
  const isRightLimb = g === 'limb' && cx >= 440;
...
function pickPort(node, role, other) {
  ...
  const ox = other.x + NODE_SIZE.w / 2;
  const oy = other.y + NODE_SIZE.h / 2;
  const cx = node.x + NODE_SIZE.w / 2;
  const cy = node.y + NODE_SIZE.h / 2;
...
function renderPortHints(node) {
  ...
  return sides
    .map((side) => {
      ...
      return `<circle class="lab-port ${cls}" cx="${p.x}" cy="${p.y}" r="3.5"
  ...
  const wires = edges
    .map((e) => {
      ...
      const title = rev
        ...
        : `${e.from} → ${e.to}`;
      return `<path class="lab-wire${bidirectional ? ' lab-wire-paired' : ''}" d="${d}"
        stroke="${cat.color}" stroke-width="2" fill="none" stroke-dasharray="${dash}"
        marker-end="url(#lab-arrow)" data-edge-id="${e.id}" data-from="${e.from}" data-to="${e.to}">
...
function renderNodeEditor() {
  ...
  root.innerHTML = `
      ...
      ${
        linkFromRuntime
          ? `<button type="button" class="btn btn-secondary btn-compact" id="lab-toggle-return">${
              hasReturn ? 'Quitar retorno ↩' : 'Añadir retorno ↩'
            }</button>`
          : ''
      }
    </div>
    ${
      state.selectedId
        ? '<p class="sub lab-port-hint">Puertos: <span class="lab-port-out-inline">●</span> salida · <span class="lab-port-in-inline">●</span> entrada (al seleccionar)</p>'
        : ''
    }
  `;
...
function toggleReturnEdge(nodeId) {
  ...
  if (reverse) {
    ...
    flashStatus(`Retorno quitado: ${node?.label ?? nodeId} → Motor`, 'info', 3500);
  } else if (forward) {
    state.edges.push({
      id: `e-ret-${Date.now()}`,
      from: nodeId,
    ...
    flashStatus(`Retorno añadido: ${node?.label ?? nodeId} → Motor`, 'ok', 3500);
  }
  saveDraft();
  renderSchematic();
  renderNodeEditor();
...
function renderAssistant() {
  ...
  const tips = [
    ...
    '<em>Cables:</em> curvas con flecha (out→in). Ida y vuelta = dos trazos paralelos — prueba «Añadir retorno ↩».',
  ];
...
async function probeBridgeStatus() {
  for (const base of bridgeUrls()) {
    try {
      ...
      const json = await res.json();
      if (res.ok && json.ok) return { online: true, base };
    } catch {
      /* try next */
    }
  }
...
function setBridgeStatus({ online, base }) {
  const el = document.getElementById('lab-bridge-status');
  if (!el) return;
  el.className = `sub lab-bridge-status ${online ? 'ok' : 'off'}`;
  el.textContent = online
    ? `Bridge OK (${base}) — puedes aplicar wiring`
    : 'Bridge offline — cd runtime && npm run lab:bridge';
}
...
async function handleApplyWiring() {
  const applyBtns = ['lab-apply-wiring', 'lab-apply-wiring-side']
    .map((id) => document.getElementById(id))
    .filter(Boolean);
  applyBtns.forEach((btn) => setButtonState(btn, 'busy', 'Aplicando…'));
  ...
  try {
    ...
    renderNodeEditor();
    renderIntentPreview();
    applyBtns.forEach((btn) => setButtonState(btn, 'ok', '✓ Aplicado'));
    flashStatus(
      result.applied
    ...
    applyBtns.forEach((btn) => resetButtonState(btn, 2800));
  } catch (e) {
    applyBtns.forEach((btn) => setButtonState(btn, 'err', '✗ Falló'));
    flashStatus(`❌ ${e.message ?? e}. ¿lab:bridge en marcha?`, 'err', 12000);
    applyBtns.forEach((btn) => resetButtonState(btn, 3500));
  }
  await refreshBridgeStatus();
}
...
function bindGlobalActions() {
  for (const id of ['lab-apply-wiring', 'lab-apply-wiring-side']) {
...
function flashStatus(msg, type = 'info', ms = 5000) {
  ...
  if (msg && type !== 'err') {
    flashStatusTimer = setTimeout(() => {
      if (toast) toast.classList.remove('lab-toast-visible');
    }, ms);
  }
}
