...
function updatePopoverLiveChrome(node) {
  ...
  const alertsRoot = pop.querySelector('#lab-popover-alerts');
  const alertsHtml = alertsRoot?.innerHTML ?? '';
  notices.innerHTML = `${banner}<div id="lab-popover-alerts" class="lab-popover-alerts" aria-live="polite">${alertsHtml}</div>`;
