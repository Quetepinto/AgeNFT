...
export const TAGS = {
  ...
  ipfsPin: {
    emoji: '📌',
    label: 'IPFS',
    hint: 'Content-addressed; pinning opcional de pago',
  },
  arweave: {
    emoji: '🗄️',
    label: 'Arweave',
...
const OPTION_META = {
  ...
  'toju-local': ['essentialE1', 'free', 'selfhost', 'privacyHigh', 'optionalOrgan'],
  'ipfs-primary': [
    'essentialE1',
    'paid',
    'x402proto',
  ...
  'w3stor-fallback': ['paid', 'decentralized', 'w3stor', 'x402proto', 'failover', 'hybridService'],
...
const DEFAULT_NODES = [
  { id: 'nft', label: 'NFT · TBA', group: 'core', x: 40, y: 48, category: 'alive', option: 'base-mainnet' },
  { id: 'runtime', label: 'Motor', group: 'core', x: 180, y: 48, category: 'alive', option: 'hermes' },
  { id: 'brain', label: 'Cerebro', group: 'core', x: 320, y: 48, category: 'alive', option: 'tx402' },
  { id: 'memory', label: 'Memoria', group: 'core', x: 460, y: 48, category: 'partial', option: 'toju-local' },
  { id: 'doctor', label: 'Doctor Qi', group: 'core', x: 600, y: 48, category: 'alive', option: 'probe' },
  { id: 'gateway', label: 'Gateway chat', group: 'edge', x: 120, y: 168, category: 'partial', option: 'telegram' },
  { id: 'chatweb', label: 'Chat web', group: 'edge', x: 280, y: 168, category: 'test', option: 'chat-api-local' },
  { id: 'matrix', label: 'Matrix', group: 'edge', x: 440, y: 168, category: 'test', option: 'matrix-bot' },
  { id: 'presence', label: 'Presencia', group: 'edge', x: 600, y: 168, category: 'partial', option: 'uruiru-svg' },
  { id: 'studio', label: 'Lab Studio', group: 'edge', x: 760, y: 168, category: 'test', option: 'lab-mvp' },
  { id: 'senses', label: 'Sentidos', group: 'edge', x: 320, y: 248, category: 'idea', option: 'stt-ocr' },
  { id: 'mas', label: 'MAS / Element X', group: 'edge', x: 520, y: 248, category: 'idea', option: 'syn2mas' },
];
...
const DEFAULT_EDGES = [
  ...
  { id: 'e4', from: 'runtime', to: 'doctor', category: 'alive' },
  { id: 'e5', from: 'runtime', to: 'gateway', category: 'alive' },
  { id: 'e6', from: 'runtime', to: 'chatweb', category: 'test' },
  { id: 'e7', from: 'runtime', to: 'matrix', category: 'test' },
  { id: 'e8', from: 'runtime', to: 'presence', category: 'partial' },
  { id: 'e9', from: 'runtime', to: 'studio', category: 'test' },
  { id: 'e10', from: 'runtime', to: 'senses', category: 'idea' },
...
const NODE_OPTIONS = {
  ...
  memory: ['toju-local', 'ipfs-primary', 'w3stor-fallback', 'arweave', 'export-only'],
...
function loadDraft() {
  try {
    ...
    if (raw) {
      ...
      return {
        nodes: parsed.nodes ?? structuredClone(DEFAULT_NODES),
...
function slugOption(detail) {
  ...
  if (d.includes('toju')) return 'toju-local';
  if (d.includes('ipfs')) return 'ipfs-primary';
...
function inferTagsForCustomOption(optionId, nodeId) {
  ...
  } else if (/ipfs|toju|w3stor|x402|arweave|pinata/.test(s)) {
    add('decentralized', 'paid', 'x402proto', 'privacyHigh', 'transferable', 'walletOnly', 'tbaPayer');
    if (/ipfs|pin/.test(s)) add('ipfsPin');
    if (/arweave/.test(s)) add('arweave');
    if (/w3stor/.test(s)) add('w3stor');
...
function renderSchematic() {
  ...
  const lines = state.edges
    .map((e) => {
      ...
      const x1 = a.x + NODE_SIZE.w / 2;
      const y1 = a.y + NODE_SIZE.h;
      const x2 = b.x + NODE_SIZE.w / 2;
      const y2 = b.y;
      const dash =
    ...
    .map((n) => {
      ...
      <g class="lab-node${selected}" data-node-id="${n.id}" tabindex="0" role="button"
        ...
        <text x="${n.x + NODE_SIZE.w / 2}" y="${n.y + 36}" text-anchor="middle"
          class="lab-node-opt">${escapeHtml(n.option)}</text>
...
function contextualTip(node) {
  const q = `¿Qué te parece cablear <code>${escapeHtml(node.label)}</code> con <code>${escapeHtml(node.option)}</code> (${CATEGORIES[node.category].label})?`;
  const map = {
    ...
    memory: 'Bloque 3.2 — IPFS/toju como primary.',
  };
...
function buildChatMarkdown() {
  ...
  for (const n of state.nodes) {
    ...
    const tags = tagsForOption(n.option)
      ...
      .filter(Boolean)
      .join(', ');
    lines.push(`- **${n.label}** → \`${n.option}\` ${tagEmojis(n.option)} · _${cat.label}_ · ${tags}`);
  }
  ...
  if (experiments.length) {
    ...
    for (const n of experiments) {
      lines.push(`- ${n.label}: \`${n.option}\` (${CATEGORIES[n.category].label})`);
    }
  }
