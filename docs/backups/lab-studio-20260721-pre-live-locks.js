...
function renderAlertHtml(alert) {
  return `<div class="lab-alert lab-alert-${alert.level}" role="alert" data-alert-id="${escapeAttr(alert.id)}">
...
function renderFeasibilityAlerts(focusNodeId = null) {
  ...
  const visibleFocus = focusNodeId ? visibleFeasibilityAlerts(focusNodeId) : [];

  if (globalRoot) {
    globalRoot.innerHTML = visibleAll.map(renderAlertHtml).join('');
  }
  if (popoverRoot) {
    popoverRoot.innerHTML = visibleFocus.map(renderAlertHtml).join('');
  }
  if (toolbar) {
    toolbar.hidden = visibleAll.length === 0;
  }
...
function renderOrganStatusPanel(node) {
  ...
  root.innerHTML = `
    ...
    ${
      stepsHtml
        ? `<details class="lab-organ-steps" open><summary>Completar configuración</summary><ol>${stepsHtml}</ol></details>`
        : ''
    }
  `;
}
...
function renderAssistant() {
  ...
  if (node) {
    ...
    tips.push(
      ...
      `<em>Estado y pasos de configuración → panel del módulo (clic en el órgano).</em>`,
      contextualTip(node)
    );
  } else {
