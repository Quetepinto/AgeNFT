...
1. **El personaje lo define el creador** — `image`, `portrait` y probablemente `voiceId` son **inmutables** por token; el owner no elige estética en el mint. → [`character-identity.md`](character-identity.md)
2. **La imagen del NFT es sagrada** — se parte de `image` / `portrait` del manifiesto; no se inventa otro personaje ni se sustituye post-mint.
3. **Movimiento es otra capa** — si se mueve (P1–P4) o queda fijo (P0) no cambia **quién** es el personaje.
4. **Tres modos en cascada** — si el modo alto falla, baja al siguiente (nunca pantalla negra).
5. **Movimiento continuo por defecto** — vídeo en bucle o animación ligera cuando no habla (si el tier lo permite).
6. **Tiempo real solo si se puede** — según presupuesto (Reflejos), GPU y red.
7. **Motor agnóstico** — Hermes y Eliza envían el mismo payload a Presencia.
8. **Contexto de hábitat** — la misma cara cambia según dónde se muestra (marketplace = foto; app = vivo). → [`presence-context-layers.md`](presence-context-layers.md)
