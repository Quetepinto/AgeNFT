...
**Hermes y OpenClaw** son candidatos **Capa 1** — ambos pueden exponer Telegram/cron/skills; el manifiesto declara cuál usa **esta instancia**. El **mint wizard** es el momento donde el owner elige motor + gateways de chat.
...
### Paso 1 — Identidad (owner)
- Nombre del agente (opcional personalizar sobre preset de colección)
- Descripción corta
- Perfil presupuesto: **A dormante / B conversacional / C activo** → [`spending-budgets.md`](../architecture/spending-budgets.md)

**No en este paso:** imagen, avatar, voz ni diseño del personaje — lo asigna el **creador** al generar el manifiesto (`image`, `presence.portrait`, `voiceId`). Ver [`character-identity.md`](character-identity.md).
...
**Orden sugerido en UI** (soberano primero):

| Gateway | Tier | Default wizard |
|---------|------|----------------|
| ☑ **Nostr** | S1 | ☑ recomendado |
| ☑ **Matrix** | S1 | ☑ recomendado |
| ☐ **Simplex** | S1 | ☐ |
| ☑ **Telegram** | S2 | ☑ (MVP; aviso teléfono owner al crear bot) |
| ☐ Discord | S2 | ☐ |
| ☐ WhatsApp | S3 | ☐ + aviso |
| ☐ Signal | S3 | ☐ oculto o desaconsejado |

→ `gateways.chat[]`, `gateways.policy`

**Nota:** credenciales (bot token, nsec) → **runtime only** tras mint; wizard solo marca `enabled: true`.

### Paso 6 — Presencia (solo calidad de render, opcional)

El **personaje** ya viene fijado en el token. El owner, si acaso, solo toca **cómo se muestra** (no la cara):

- Max tier permitido: static … emotion (según preset colección)
- TTS activo sí/no (la **voz del personaje** sigue siendo la del creador)

→ `organs.presence.maxTier`, flags de render — **no** `portrait` ni `voiceId`
...
|-------------|-------------------|
| Identidad (owner) | `name`, `description`, `budget.profile` |
| Personaje (creador, pre-mint) | `image`, `organs.presence.portrait`, `idleAsset`, `tts.voiceId` |
