...
### Entrega y transcode vídeo — Livepeer

Red descentralizada de streaming/transcode. **No genera** la cara — **entrega** el stream al usuario y convierte assets.

→ Spec completa: [`livepeer-video-habitat.md`](livepeer-video-habitat.md)

| Producto | Uso ageNFT | Agente-soberano |
|----------|------------|-----------------|
| [Livepeer Studio](https://livepeer.studio/) | Lab: HLS, transcode `previewAsset` | ❌ cuenta API |
| [Red Livepeer](https://livepeer.org/) | Prod: gateway + avatar IA stream | ⚠️ ETH Arbitrum |

**Combo:** MuseTalk local → ingest Livepeer → app móvil; voz por **dTelecom** (Base x402).
